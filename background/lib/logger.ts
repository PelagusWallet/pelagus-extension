...
class Logger {
  ...
  logEvent(level: Exclude<LogLevel, LogLevel.off>, input: unknown[]): void {
    ...
    // Check if any of the input is an Error object with a stack trace
    const errorWithStack = input.find((item) => item instanceof Error && item.stack)
    let stackTrace: string[] | undefined

    if (errorWithStack) {
      // Use the original error's stack trace
      stackTrace = (errorWithStack as Error).stack
        ?.split("\n")
        ?.filter((line) => {
      ...
      // Fall back to generating a new stack trace
    ...
    if (this.isConsoleLoggingEnabled) {
      ...
      // Always show stack traces in console if available
