const tokenIcons = {
  AAVE: ["https://cryptologos.cc/logos/thumbs/aave.png?v=022"],
  UNI: ["ipfs://QmXttGpZrECX5qCyXbBQiqgQNytVGeZW5Anewvh2jc4psg"],
  crvCVXETH: [
    "https://assets.coingecko.com/coins/images/279/small/ethereum.png?1595348880",
    "ipfs://bafybeicpyu35mpptkxvbcfbzmlaefiqlj7layt5oolcbjh5cc5vfkyi7c4",
  ],
  "YFIETH-f": [
    "https://assets.coingecko.com/coins/images/279/small/ethereum.png?1595348880",
    "https://cryptologos.cc/logos/thumbs/yearn-finance.png?v=022",
  ],
  SNX: [
    "https://raw.githubusercontent.com/trustwallet/assets/master/blockchains/ethereum/assets/0xC011a73ee8576Fb46F5E1c5751cA3B9Fe0af2a6F/logo.png",
  ],
  SUSHI: ["https://cryptologos.cc/logos/thumbs/sushiswap.png?v=022"],
  LOOKS: ["ipfs://bafybeia64nloazn7cxgwema5o26l2zjbxufb5kb5dxea6dbpretuxoappq"],
  KEEP: ["https://s2.coinmarketcap.com/static/img/coins/64x64/5566.png"],
  crvTETH: [
    "https://assets.coingecko.com/coins/images/279/small/ethereum.png?1595348880",
    "ipfs://bafybeib53hx56um4xexwtaqjo3u3ridppnpblfucqdbotwlqcadlfyc2a4",
  ],
}

export default tokenIcons
