...
const QiReservationAllocationPage = (): ReactElement => {
  const dispatch = useBackgroundDispatch()
  const history = useHistory()
  const request = useBackgroundSelector(
    (state) => state.qiReservation.allocationRequest
  )
  const { t } = useTranslation("translation", {
    keyPrefix: "drawers.transactionConfirmation.qiReservationAllocation",
  })
  ...
  useEffect(() => {
    const rejectPendingRequestOnExit = () => {
      ...
      dispatchRef.current(rejectQiReservationAllocation({ requestId }))
    }
  ...
  const rejectRequest = () => {
    ...
    dispatch(rejectQiReservationAllocation({ requestId })).finally(closePopup)
  }

  const reserveAddresses = async () => {
    const requestId = request?.requestId
    ...
    try {
      const result = (await dispatch(
        confirmQiReservationAllocation({ requestId })
      )) as ThunkResult | undefined
      if (result?.error) {
        setError(result.error.message || t("confirmError"))
  ...
  return (
    <main className="allocation-wrapper">
      <SharedGoBackPageHeader
        title={t("headerTitle")}
        onBack={rejectRequest}
        preventNavigation
      />
      ...
      {request ? (
        <div className="content">
          <header>
            ...
            <h1>{t("title", { count: request.count })}</h1>
            <p className="origin">
              {t("requestedBy", { origin: request.origin })}
            </p>
          </header>

          <p className="summary">{t("summary", { count: request.count })}</p>

          <section className="warning" aria-labelledby="warning-title">
            <h2 id="warning-title">{t("capacityTitle")}</h2>
            <p>{t("capacityBody", { count: request.count })}</p>
      ...
      <SharedActionButtons
        title={{
          confirmTitle: request ? t("reserveButton") : t("closeButton"),
          cancelTitle: t("rejectButton"),
        }}
        onClick={{
          onConfirm: request ? reserveAddresses : closePopup,
...
export default QiReservationAllocationPage
