...
const PELAGUS_ICON_URL =
  "https://pelaguswallet.io/icons/icon-144x144.png?v=41306c4d4e6795cdeaecc31bd794f68e"

const PELAGUS_NAME = "Pelagus"
...
const moreThanOneWalletInstalledAndPelagusIsNotDefault = (): boolean => {
  if (
    window.ethereum &&
  ...
  // Otherwise - if the user only has Pelagus installed - or if they have multiple
  // wallets installed and have Pelagus as their default wallet - return false
  return false
}
...
const findAndReplaceUniswapInjectedOption = (): void => {
  ...
  if (
    ...
    if (iconAndTextDiv && iconAndTextDiv.innerHTML.includes("Injected")) {
      // Replace the arrow icon with the Pelagus icon
      iconAndTextDiv.innerHTML = iconAndTextDiv.innerHTML.replace(
        /\ssrc="(.+)"\s/,
        ` src="${PELAGUS_ICON_URL}" `
      )

      // Replace the `Injected` text with `Pelagus`
      iconAndTextDiv.innerHTML = iconAndTextDiv.innerHTML.replace(
        "Injected",
        PELAGUS_NAME
      )
...
const findAndReplaceJoeMetamaskOption = (addedNode: Node): void => {
  ...
  if (textNode && img) {
    textNode.textContent = PELAGUS_NAME
    img.src = PELAGUS_ICON_URL
  }
}
  ...
  if (
    ...
    for (const option of connectionOptions) {
      if (option.classList.contains("MetaMask-btn")) {
        option.innerHTML = option.innerHTML.replaceAll(METAMASK, PELAGUS_NAME)
        // Replace metamask icon with Pelagus icon
...
const createPelagusImg = (): HTMLImageElement => {
  const pelagusIcon = document.createElement("img")
  pelagusIcon.src = PELAGUS_ICON_URL
  pelagusIcon.setAttribute("height", "48px")
  pelagusIcon.setAttribute("width", "48px")
  return pelagusIcon
}
    ...
    for (const element of parentElement) {
      ...
      if (textContainer?.innerHTML === INJECTED) {
        ...
        if (textContainer && iconContainer) {
          textContainer.textContent = PELAGUS_NAME

          iconContainer.removeChild(iconContainer.children[0])
          iconContainer.appendChild(createPelagusImg())
    ...
    for (const li of addedNode.getElementsByTagName("li")) {
      if (li.innerText === text) {
        ...
        if (textContainer && iconContainer) {
          ...
          const pelagusIcon = document.createElement("img")
          pelagusIcon.src = PELAGUS_ICON_URL
          pelagusIcon.setAttribute("height", "24px")
          pelagusIcon.setAttribute("width", "24px")
          iconContainer.removeChild(iconContainer.children[0])
          iconContainer.appendChild(pelagusIcon)
          iconContainer.appendChild(iconContainer.children[0])
        }
      }
    }
...
function findAndReplaceCelerMetamaskOption(addedNode: Node): void {
  ...
  if (addedNode instanceof HTMLElement) {
    ...
    /* Adding a Pelagus icon after login in the account view */
    if (
      ...
      for (const btn of addedNode.getElementsByTagName("button")) {
        if (btn.innerText === METAMASK) {
          ...
          if (textContainer && img) {
            ...
            btn.appendChild(createPelagusImg())
...
function addpelagusButtonForWalletConnectModal(addedNode: Node): void {
  if (!(addedNode instanceof HTMLElement)) {
    return
  }
  ...
  const pelagusButton = injectedUI.querySelector("button")
  if (pelagusButton) {
    pelagusButton.onclick = () => {
...
export default function monitorForWalletConnectionPrompts(): void {
  ...
  if (process.env.SUPPORT_WALLET_CONNECT === "true") {
    observeMutations(addpelagusButtonForWalletConnectModal)
  }
}
